...
/*   Updated: 2023/11/01 20:54:42 by jlaazouz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_square(t_data *data, int color, int cub)
{
	...
	while (j <= cub)
	{
		while (i <= cub)
		{
			if (data->x_start + i < data->minimap.width && data->y_start
				+ j < data->minimap.height)
				mlx_put_pixel(data->image, data->x_start + i, data->y_start + j,
					color);
			i++;
		}
		i = 0;
