...
/*   Updated: 2023/10/30 16:15:30 by jlaazouz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_line_texture(t_data *data, mlx_texture_t *tex, int top, int bottom)
{
    ...
    while (window_y < data->window_height)
    {
        ...
        if (data->var.i >= 0 && data->var.i < data->window_width 
            ...
            mlx_put_pixel(data->image, data->var.i, window_y, data->texture.color);
         tex_y += inc;
         window_y++;
    }
    // printf ("i : %d\n", i);
...
void    draw_scene_line(t_data *data, int top, int bottom, int axe)
{
    ...
    while (top >= 0 && y < top && top < data->window_height)
        if (data->var.i >= 0 && data->var.i < data->window_width && y >= 0 && y < data->window_height)
            mlx_put_pixel(data->image, data->var.i, y++, data->visuals->ceiling_color);
        y++;
    y = bottom;
    while (y >= 0 && y < data->window_height && bottom < data->window_height)
        if(data->var.i >= 0 && data->var.i < data->window_width && y >=0 && y < data->window_height)
            mlx_put_pixel(data->image, data->var.i , y++ , data->visuals->floor_color);
}